...
from workpilot.providers.base import EvidenceType
...
def valid_evidence(workspace: WorkspaceTools) -> EvidenceStore:
    ...
    store.insert(Evidence(
        evidence_id="E-0001",
        locator=SourceLocator.for_file_lines("meeting_notes.md", 12, 12),
        quote="支付重试逻辑的 API 设计仍未确定，李四需要本周给出方案，否则下游开发将被阻塞。",
        evidence_type=EvidenceType.RISK,
    ))
    store.insert(Evidence(
        evidence_id="E-0002",
        locator=SourceLocator.for_file_lines("meeting_notes.md", 18, 18),
        quote="支付重试方案截止日期定为本周五（6月27日）。",
        evidence_type=EvidenceType.DECISION,
    ))
    store.insert(Evidence(
        evidence_id="E-0003",
        locator=SourceLocator.for_file_lines("pr_summary.md", 7, 7),
...
def test_quote_mismatch_fails(workspace: WorkspaceTools) -> None:
    ...
    store.insert(Evidence(
        evidence_id="E-0001",
        locator=SourceLocator.for_file_lines("meeting_notes.md", 1, 1),
...
def test_source_file_not_found_fails(workspace: WorkspaceTools) -> None:
    ...
    store.insert(Evidence(
        evidence_id="E-0001",
        locator=SourceLocator.for_file_lines("nonexistent_file.md", 1, 1),
        quote="some quote",
        evidence_type=EvidenceType.CONTEXT,
    ))
...
def test_line_range_mismatch_warns(workspace: WorkspaceTools) -> None:
    ...
    store.insert(Evidence(
        evidence_id="E-0001",
        locator=SourceLocator.for_file_lines("meeting_notes.md", 1, 2),
        quote="支付重试方案截止日期定为本周五（6月27日）。",
        evidence_type=EvidenceType.DECISION,
    ))
